...
| `src/engine.js` | `computeDeterministicSeed()`, `calculateLiveMetrics()`, `generateDuoName(uName, pName, rollIndex, userProfile, partnerProfile)` — picks a thematic PAIR CONCEPT (Sun/Moon, Salt/Pepper, ...) from `PAIR_CONCEPTS` instead of mashing letters together; returns `{label, userHalf, partnerHalf, line}`. Concepts marked `energy: true` assign halves by MBTI E/I (extrovert gets the bright/outward half) when both profiles have one and they differ; everything else uses a stable per-couple hash tiebreak so the SAME concept always assigns the same way on reroll. `generateSoloVibeName()` (returns "Alex Soleil" style), `deriveWyrPersonalityLabel()`. |
