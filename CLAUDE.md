...
| `src/composer.js` | The variety engine. `accountSalt()` (hash of vibeSeed + save code — per-ACCOUNT entropy mixed into every content seed so two accounts created with identical onboarding answers never see identical content), `hashStr()`, `combineSeeds()` (plain sum — good for cycling a pool by +1 per tap, never for a coin-flip), `pickFrom()`, `pickVariant()` (tolerates string or array entries — all one-line-per-key tables are now arrays of variants, picked with a per-surface tag so the glance/decoder/vibe never echo the same line on the same day). `rollChance(pct, ...seeds)` — the probability-roll primitive: pipes `combineSeeds()` through an integer avalanche finalizer (Murmur3-style, `mixInt()`) before the modulo check. Without this, adding a slowly-incrementing counter (a refresh tap, a day-index delta) into a plain sum barely moves it, so a "30% chance" either fires on EVERY consecutive tap/day or NEVER — see Known Bugs. Humor system: `HUMOR_LEVELS` (chill/playful/unhinged, persisted `gd.settings.humorLevel`, UI in Profile Settings), `kickerFor(kind, ...seeds)` (appends a joke to ~30-45% of messages via `rollChance`; 'pet' kind uses the pet banks with a `{pet}` token; dark jokes ONLY at 'unhinged' AND never on low/tense mood days via `darkHumorOK()`), `maybeRareLine()` (~1-in-50 days a collectible line appears — shiny-pet pattern for messages). `maybeToughLine(pool, force, ...seeds)` / `toughLineOK()` — a blunter, critical counterweight shown at EVERY humor level (~25% unforced), independent of the joke system; `force: true` (the "🎯 Straight Talk" button) guarantees one, still suppressed on low/tense mood days. |
| `src/jokes.js` | Jokes content pack: `KICKERS_PLAYFUL`/`KICKERS_DARK` (general), `PET_KICKERS_PLAYFUL`/`PET_KICKERS_DARK` ({pet} token), `RARE_LINES`, `TOUGH_LOVE_LINES` (blunt/critical, not a joke — shows at every humor level), `MILESTONE_TOASTS` (+ `MILESTONE_TOAST_GENERIC`). Dark lines punch at the universe/entropy/group chats, never at the user's insecurities — keep it that way. |
...
| `src/engine.js` | `computeDeterministicSeed()`, `calculateLiveMetrics()`, `generateDuoName(uName, pName, rollIndex, userProfile, partnerProfile)` — picks a thematic PAIR CONCEPT (Sun/Moon, Salt/Pepper, ...) from `PAIR_CONCEPTS` instead of mashing letters together; returns `{label, userHalf, partnerHalf, line, why}`. Every concept is tagged with a personality `axis` (energy/attachment/conflict/loveLanguage/expression/mbtiJP/general, 27 concepts total) that both decides the half-assignment rule and drives `REASON_BUILDERS[axis]` — a "why" sentence built from the two people's ACTUAL trait values (e.g. "Sam feels loved through undivided time; Alex through kind words — different recipes, same result"), not filler text. `energy`-axis concepts assign halves by MBTI E/I (extrovert gets the bright/outward half) when both profiles have one and differ; everything else uses a stable per-couple hash tiebreak so the SAME concept always assigns the same way on reroll. `generateSoloVibeName()` (returns "Alex Soleil" style), `deriveWyrPersonalityLabel()`. |
...
| `src/games/redflag.js` | Red Flag Bingo — playful self-roast board (solo) / household flags (partner) on the sparks-grid CSS. Deterministic seeded boards (`boardsCompleted` + account salt) rotate per completion. Full board = growth (cap id `redflags`) + `redflag_board` milestone. |
| `src/games/pettycourt.js` | Petty Court — the pet rules on petty disputes. Partner: pass-the-phone double-blind picks → verdict; agreement rate in `gd.pettycourt`. Solo: you vs. deterministic per-question "public opinion" (`publicSide()` — precedent never flip-flops). Cap id `pettycourt`. |
| `src/games/calledit.js` | Called It — one locked prediction about the week, resolvable (self-reported) after 3 local days; lifetime track record in `gd.calledit`. The app's only deliberate come-back-later loop. Cap id `calledit`. |
| `src/games/capsule.js` | Time Capsule — one line to future-you, sealed 30 local days (`gd.capsule.entries`, last 12). Exports `readyCapsule()`; the Journey drawer echoes when one is openable. Cap id `capsule`. |
| `src/games/quicktakes.js` | 3-question rapid-fire round. Updates `wyr.preferences`. Awards pet growth per session (daily cap). |
| `public/empal.html` | **Unrelated bonus page** — patch notes + connect info for the Em-Pal Palworld server the app owner runs on the side. Deliberately a fully standalone static HTML file (own inline CSS/JS, zero imports from `src/`), NOT part of the SPA/build pipeline at all — reachable directly at `/empal.html`, shareable as a direct link with zero AssTrollOGs profile/onboarding required. Pure read-only display: fetches the current `server_address`/`password` from the `em_pal_config` Supabase table (own inline `createClient` via an `esm.sh` CDN import, not the app's bundled client) and falls back to hardcoded last-known values if that fetch ever fails, so the page never breaks. Address and port are shown as ONE combined field (`1.2.3.4:8211`) since that's what players actually paste into Palworld's connect dialog. Password starts masked with a tap-to-reveal button. The app links to it from two places so it's reachable everywhere, not just the dashboard (`.empal-top-link` in styles.css) — both real `<a href="/empal.html">` hyperlinks, not JS-driven, so neither can affect app state: one sits between the sticky header and the screens in `index.html` (outside both `#profile-screen` and `#daily-screen`, and deliberately outside `#dynamic-header` itself since that element's innerHTML gets fully replaced by JS on every screen change — a link placed inside it would get wiped out), covering onboarding and the dashboard; the other sits in `.drawer-handle`, the one bit of drawer chrome shared by every drawer type (`openDrawer()` in `drawers.js` only ever swaps `#drawer-dynamic-content`), covering every game/insight/settings drawer. No edit UI on this page at all — see below. Earlier iterations tried an in-app "screen" reached via hash-routing with a JS-driven back button (reverted, needless complexity), then an edit form directly on this page (reverted — moved to the dev panel instead, see below). |
| Em-Pal editing | Lives entirely in the app's existing dev panel (`src/dev-tools.js`'s "Em-Pal Server" fields, gear icon → `Calgary1!`), NOT on `public/empal.html` — that page only ever reads. `devSaveEmPalConfig()` writes `{server_address, password}` to the shared `em_pal_config` Supabase table via `supabase.js`'s `getEmPalConfig()`/`saveEmPalConfig()`; fields auto-populate with the current values whenever the panel unlocks. Same trust model as `user_sessions`: RLS allows open read AND write (no extra server-side password check) because the dev panel's own client-side gate already is this app's established security model for every other dev-tools action — see `supabase/migrations/…create_em_pal_config.sql` (also folded into `supabase/bootstrap.sql`). **That migration must be run once in the Supabase SQL Editor before the dev panel fields or the live page read will work** — same requirement as every other migration in this repo. |
...
```js
{
  ...
  settings:   { humorLevel: 'chill'|'playful'|'unhinged' },   // joke kickers; dark only at unhinged
  redflag:    { checkedCells: [], lastPlayed, boardsCompleted },
  pettycourt: { cases, agreements, lastPlayed },
  calledit:   { active: {date, q, pick} | null, made, right, lastPlayed },
  capsule:    { entries: [{sealedAt, unlockDate, text, opened}] /* last 12 */, lastPlayed },
  petq:       { lastAnswered /* local date */, count, lastChoice },   // daily pet question
...
- Games with daily cap: `trivia`, `wyr`, `sparks`, `mood`, `quicktakes`, `dailyq`, `checkin`, `redflags`, `pettycourt`, `calledit`, `capsule`, `petq` (the daily pet question in the pet drawer)
...
`trivia_first`, `trivia_perfect` (≥10 total, 100%), `trivia_master` (≥15, ≥80%), `wyr_5`, `wyr_10`, `wyr_25`, `dailyq_first`, `dailyq_7`, `duo_reader` (≥10 guesses, ≥70% right), `checkin_first`, `checkin_4`, `bingo_3`, `bingo_row`, `streak_3`, `streak_7`, `mood_first`, `mood_consistent` (5-day mood streak), `quicktakes_first`, `quicktakes_pattern` (5 sessions), `pet_baby` (4d), `pet_adult` (20d), `pet_legendary` (40d), `pet_couple_shiny` (couple pet independently ≥40d, partner mode only), `redflag_board` (1st full Red Flag board), `pettycourt_first`, `pettycourt_docket` (10 cases), `calledit_first` (1st resolved prediction), `calledit_prophet` (5 correct), `capsule_first` (1st sealed), `capsule_open` (1st opened), `friend_first` (1st friend added), `friend_circle` (≥5 friends), `friend_bond` (any friendship pet reaches stage 5), `friend_streak_7` (any friendship visit streak ≥7). Legacy (labels kept, checks removed with the memory game): `memory_first`, `memory_sharp`.
...
|---|---|---|
| FIXED | Joke kickers and the "rare line" 1-in-50 check used `combineSeeds()` (a plain sum) directly in a `% 100 < chance` test. Adding a slowly-incrementing counter (the refresh-tap offset, a day-to-day date hash) into a sum barely moves it, so the roll was effectively CONSTANT across many consecutive taps/days — depending on an account's baseline, jokes appeared on every single refresh in a session or never once (confirmed live: 10/10 taps hit in one test account). This is why jokes felt "missing" — some accounts simply never rolled a hit. Fixed with `composer.mixInt()`, an integer avalanche finalizer applied before the modulo check (`rollChance()`), so small input deltas scatter across the whole output range instead of shifting it by a matching small amount. Also applied to `pet.js`'s day-based quirk-of-day check, which had the same weakness. | `composer.js`, `pet.js` |
| FIXED | Insights were uniformly warm/affirming with no critical counterweight (pet.js already had a warning/affirmation split; insights.js didn't). Added `TOUGH_LOVE_LINES` (jokes.js) + `composer.maybeToughLine()` — a blunt, critical line shown at every humor level (~25% unforced), plus a "🎯 Straight Talk" button (Day at a Glance, Blueprint, all 4 Deep Insight drawers) that forces one on demand. | `jokes.js`, `composer.js`, `insights.js`, `dashboard.js`, `drawers.js`, `index.html` |
| FIXED | Teamwork Chronicles always resolved as a clean win — no failure/dark path existed even at the Unhinged humor level. Added `CHRONICLE_DARK_SOLO_RESULTS`/`CHRONICLE_DARK_RESULTS`, rolled in ~30% of the time at Unhinged only. | `drawers.js` |
| FIXED | The Duo Generator mashed substrings of both names together into a fake portmanteau (e.g. slicing "Alex" + "Sam" into nonsense fragments) that users correctly read as meaningless. Replaced with `engine.generateDuoName()` picking a real thematic pair concept (Sun & Moon, Salt & Pepper, Fire & Ice, ...) with MBTI-informed half assignment. | `engine.js`, `drawers.js`, `dashboard.js`, `index.html` |
| FIXED | Every content seed derived purely from profile answers, so two accounts created with the same name/city/traits (someone recreating an account, testing, or genuine overlap) saw literally identical daily insights, blueprints, spotlight tips, pet names, lucky numbers, quirks, and friend vibe scores. Fixed with `composer.accountSalt()` (vibeSeed + save code entropy) mixed into every seed. | `composer.js`, `insights.js`, `pet.js`, `friends.js` |
| FIXED | Day at a Glance's rotation key used day-of-WEEK (`date.getDay()`), not the date — a month of daily opens cycled through only ~5-7 distinct reads before repeating weekly. A local-date hash now moves the rotation every calendar day (25/30 unique bodies in a simulated month, up from 5). | `insights.js` |
| FIXED | All the glance's bonus-content lanes (mood trend, MBTI flavor, time-theme, spotlight tip, game hints) gated on `fortuneOffset` alone, which is 0 on every fresh app open — a user who never tapped "New Insight" NEVER saw any of them. Gates now rotate with the daily key. | `insights.js` |
| FIXED | The same `COMBO_INSIGHTS`/`ATTACHMENT_PAIRINGS` sentence showed verbatim in the daily glance, the Decoder drawer, and the Vibe drawer on the same day (each had exactly one line per trait key). Tables are now variant arrays picked with per-surface tags. | `content-bank.js`, `insights.js` |
| FIXED | `saveProfileSettings()` wrote the JSON.parse'd cached package back with its STALE `gameData` snapshot, silently clobbering anything changed since the last save. Now reassigns `parsed.gameData = window.AppState.gameData` before writing. | `drawers.js` |
...
Every `PAIR_CONCEPTS` entry now carries an `axis` (energy/attachment/conflict/loveLanguage/expression/mbtiJP/general) instead of a bare `energy: true` flag, and the bank grew from 18 to 27 concepts (added Anchor & Sail, Root & Wing, Harbor & Horizon for attachment; Thunder & Rain, Give & Take for conflict; Honey & Tea for love language; Blueprint & Wanderer, Map & Compass for MBTI J/P; Left Sock & Right Sock for general flavor). `generateDuoName()` now also returns a `why` field from a new `REASON_BUILDERS[axis]` map — each builder reads the two profiles' REAL values for that axis (e.g. actual `attachmentStyle`s, actual `loveLanguage`s) and explains the pairing in plain language, with a same-value case ("you both...") and a different-value case ("X does _, Y does _ — different approaches, same goal") rather than one generic sentence. Displayed in a new card under the pairing tagline (`#vibe-name-why`), updated on every reroll.

Verified live: an ENFP/INTJ pair correctly produced "Sam brings the outward spark, Alex brings the steady depth" on every energy-axis concept across 6 consecutive rerolls, and the harness confirms the attachment/loveLanguage axes surface the actual trait labels (not boilerplate) — 45 checks, 0 failures.

### 2026-07-16 — Fix Streaky Probability Rolls; Tough-Love Lines; Chronicles Dark Path; Duo Generator Rebuild

Follow-up pass after live testing surfaced a real bug plus three content-quality gaps in the variety engine shipped the day before. No `SCHEMA_VERSION` bump.

**The bug.** A live browser smoke test showed jokes appearing on 10/10 consecutive "New Insight" taps — statistically near-impossible at the intended ~30% rate. Root cause: `kickerFor()`/`maybeRareLine()` fed `combineSeeds()` (a plain additive sum) straight into a `% 100 < chance` check. Adding a slowly-incrementing counter (the refresh offset, a day-to-day date hash) to a sum barely moves it, so the roll stayed constant across many consecutive taps/days — some accounts would see a joke on every tap, others on none, in either case a "streak" rather than an independent roll. New `composer.mixInt()` (Murmur3-style integer avalanche finalizer) + `rollChance(pct, ...seeds)` fix this by scattering small input deltas across the whole output range before the modulo check; `kickerFor`, `maybeRareLine`, and `pet.js`'s day-based `computeQuirkOfDay` (same weakness) all switched to it. Verified with a new harness section that checks no account hits 0/10 or 10/10 across its first 10 taps over 50 simulated accounts.

**Tough-love lines.** Insights were uniformly warm/affirming with no critical counterweight. New `TOUGH_LOVE_LINES` bank (jokes.js) + `composer.maybeToughLine()` — blunt, critical, NOT a joke (shows at every humor level including Chill, ~25% unforced) — wired into Day at a Glance, Blueprint, and all 4 Deep Insight drawers. Suppressed on low/tense mood days like dark humor.

**"🎯 Straight Talk" button.** A small addition riding on the tough-love bank: forces the next read to guarantee a tough-love line instead of leaving it to the roll — explicit user control over tone. Added next to the existing refresh buttons on Day at a Glance, Blueprint, and each Deep Insight drawer.

**Chronicles dark path.** Teamwork/Solo Chronicles always resolved as a clean win. Added `CHRONICLE_DARK_SOLO_RESULTS`/`CHRONICLE_DARK_RESULTS` — comedic-disaster outcomes rolled in ~30% of the time, gated to the Unhinged humor level only (verified live: both dark and success paths reachable).

**Duo Generator rebuild.** `engine.generateDuoName()` mashed name substrings into a meaningless portmanteau. Replaced with a curated `PAIR_CONCEPTS` bank (Sun & Moon, Fire & Ice, Salt & Pepper, Peanut Butter & Jelly, ...) — concepts marked `energy: true` assign halves by MBTI E/I (extrovert gets the bright half) when both profiles have one and differ, otherwise a stable per-couple hash tiebreak. Returns `{label, userHalf, partnerHalf, line}` (e.g. "Alex is the Moon to Sam's Sun") instead of a mashed string; reroll cycles through remaining concepts. Verified live: MBTI-informed assignment reproduced correctly (introvert → Moon, extrovert → Sun) and reroll produced distinct concepts.

Verified with `tests/content-harness.mjs` (42 checks, up from 32) plus a live Playwright smoke pass against the dev server (dev-panel fake profile, real button clicks/evaluates, screenshots) — confirmed non-streaky joke rate, forced tough-line guarantee + mood suppression, Deep Insight drawer's Straight Talk button, Duo Generator's trait-informed assignment, and both Chronicles outcome paths. `npm run build` clean.

### 2026-07-15 — Variety Engine: Account Salt, Humor System, Multi-Variant Banks, 4 New Games

Six-phase overhaul so no two accounts (even with identical onboarding answers) see the same content, and so messages/games stop repeating. No `SCHEMA_VERSION` bump — every new `gameData` field is additive and backfilled in `migrateGameData()`.

**Phase 1 — Account salt.** New `src/composer.js`. `accountSalt()` (vibeSeed — which carries onboarding-timestamp entropy — plus the random save code) is mixed into `computeContentSeed` (insights), `petFlavorSeed` (lucky number/quirk/identity), `generatePetName` (new pets only; persisted names untouched), and `computeFriendVibeScore`. Pet VISUALS still derive from the unsalted profile seed on purpose so existing pets don't change appearance.

**Phase 2 — Humor system + multi-variant banks.** `gd.settings.humorLevel` (chill/playful/unhinged; Profile Settings UI). New `src/jokes.js` kicker banks; `composer.kickerFor()` seasons ~30-45% of messages, dark only at unhinged and never on low/tense mood days; `maybeRareLine()` gives ~1-in-50-day collectible lines (glance + pet message slot); newly-earned milestones get toast lines. Every one-line-per-key table converted to variant arrays (`COMBO_INSIGHTS` ×3, `ATTACHMENT_PAIRINGS` ×3, `CONFLICT_PAIRINGS` ×3, `CONFLICT_SOLO` ×3, `COUPLE_MESSAGES` ×3 + daily rotation, pet mood reactions ×3, mood/trend/rel-status notes ×3, friend pairing tables ×2, `FRIEND_OF_DAY_TIPS` ×3) picked via `pickVariant()` with per-surface tags so surfaces sharing a table don't echo each other.

**Phase 3 — Game pools.** Trivia rebuilt on a scenario bank (3 variants per category × solo/partner phrasings, rotated by rounds+salt; tiered joke-y result pools). Daily Duo +10, Reflection +10, QuickTakes +13 (salted rotation), WYR +13 plus a 12-question **Cursed Edition** pack (dark dilemmas, unhinged-only, all with real preference signals).

**Phase 4 — New games.** Petty Court (⚖️ pet-judged petty disputes; pass-the-phone double-blind in partner mode, vs deterministic "public opinion" solo), Called It (🔮 weekly prediction, resolves after 3 local days, lifetime track record — the app's first anticipation loop), Red Flag Bingo (🚩 self-roast board on the sparks-grid CSS, deterministic seeded boards), Time Capsule (💌 note to future-you, 30-day seal, Journey-drawer echo). All follow the standard game contract; 7 new milestones registered in all three places; all count toward pet Bond.

**Phase 5 — Pet daily question + friend content.** The pet asks one deterministic weird question a day in its drawer (`gd.petq`, +1 growth, cap `petq`). Friend jokes +8, icebreakers +1/style, messages +1/language.

**Phase 6 — Regression harness.** New `tests/content-harness.mjs` (+ `tests/loader.mjs` supabase stub): salt divergence, determinism, full trait-key coverage of every variant table, token hygiene across 240 composed messages, humor/mood gating, trivia rotation, WYR signal validity (incl. cursed), milestone label/toast coverage, migration backfill, and a 30-day repeat-rate simulation — 32 checks, 0 failures. The simulation caught two real bugs (see Known Bugs): the glance rotated by day-of-week (≈5 unique reads/month) and its bonus lanes never fired without "New Insight" taps. After fixes: 25/30 unique bodies in a simulated month with zero gameplay signals. Run: `node --loader ./tests/loader.mjs tests/content-harness.mjs`.
