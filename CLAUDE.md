...
| `src/empal.js` | **Unrelated bonus section** — patch notes + connect info for the Em-Pal Palworld server the app owner runs on the side. Not part of the personality data model at all; a static drawer (`renderEmPalDrawer()`) reached from a visually-separated "🎮 Elsewhere" card at the bottom of the dashboard (dashed divider on purpose). `EM_PAL_CONNECT` is the only thing that ever needs editing — plain `{ip, port, password, discord, note}` object; leaving `ip` blank shows a "coming soon" card instead. Connect fields reuse the save-code display-row/copy-button styling (`copyEmPalField()`). No auth gate: the connect info (including password) is visible to anyone who opens the app or reads the repo, by design, since the goal is "friends can find it easily" — worth revisiting if that stops being the intent. |
