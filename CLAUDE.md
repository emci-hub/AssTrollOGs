...
| `src/composer.js` | The variety engine. `accountSalt()` (hash of vibeSeed + save code — per-ACCOUNT entropy mixed into every content seed so two accounts created with identical onboarding answers never see identical content), `hashStr()`, `combineSeeds()`, `pickFrom()`, `pickVariant()` (tolerates string or array entries — all one-line-per-key tables are now arrays of variants, picked with a per-surface tag so the glance/decoder/vibe never echo the same line on the same day). Humor system: `HUMOR_LEVELS` (chill/playful/unhinged, persisted `gd.settings.humorLevel`, UI in Profile Settings), `kickerFor(kind, ...seeds)` (appends a joke to ~30-45% of messages; 'pet' kind uses the pet banks with a `{pet}` token; dark jokes ONLY at 'unhinged' AND never on low/tense mood days via `darkHumorOK()`), `maybeRareLine()` (~1-in-50 days a collectible line appears — shiny-pet pattern for messages). |
| `src/jokes.js` | Jokes content pack: `KICKERS_PLAYFUL`/`KICKERS_DARK` (general), `PET_KICKERS_PLAYFUL`/`PET_KICKERS_DARK` ({pet} token), `RARE_LINES`, `MILESTONE_TOASTS` (+ `MILESTONE_TOAST_GENERIC`). Dark lines punch at the universe/entropy/group chats, never at the user's insecurities — keep it that way. |
