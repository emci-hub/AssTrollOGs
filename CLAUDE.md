...
| `public/empal.html` | **Unrelated bonus page** — patch notes + connect info for the Em-Pal Palworld server the app owner runs on the side. Deliberately a fully standalone static HTML file (own inline CSS/JS, zero imports from `src/`), NOT part of the SPA/build pipeline at all — reachable directly at `/empal.html`, shareable as a direct link with zero AssTrollOGs profile/onboarding required. To update server info, edit the three values (`#ip`, `#port`, `data-real` on `#password`) directly in the file — that's the only thing that ever needs to change. Password starts masked with a tap-to-reveal button (plain inline JS, no framework). The dashboard has one small `<a href="/empal.html">` text link at the top of the daily screen (`.empal-top-link` in styles.css) — a real hyperlink, not JS-driven, so it can't affect app state. No auth gate on the connect info: visible to anyone with the link or reading the repo, by design ("friends can find it easily") — worth revisiting if that stops being the intent. Earlier iterations tried making this an in-app "screen" reached via hash-routing (`#empal`) with a JS-driven back button; that added real complexity for zero benefit over a plain static page and was reverted — keep it a static file. |
